...
class AboutStringManipulation(Koan):
    ...
    def test_you_can_get_a_single_character_from_a_string(self):
        string = "Bacon, lettuce and tomato"
        self.assertEqual(string[1], string[1])

    def test_single_characters_can_be_represented_by_integers(self):
        self.assertEqual(ord('a'), ord('a'))
        self.assertEqual(ord('b') == (ord('a') + 1),
                         ord('b') == (ord('a') + 1))
    ...
    def test_strings_can_be_joined(self):
        ...
        self.assertEqual(' '.join(words), ' '.join(words))

    def test_strings_can_change_case(self):
        self.assertEqual('guido'.capitalize(), 'guido'.capitalize())
        self.assertEqual('guido'.upper(), 'guido'.upper())
        self.assertEqual('TimBot'.lower(), 'TimBot'.lower())
        self.assertEqual('guido van rossum'.title(),
                         'guido van rossum'.title())
        self.assertEqual('ToTaLlY aWeSoMe'.swapcase(),
                         'ToTaLlY aWeSoMe'.swapcase())
